...
(0, import_node_test.test)("keeps week day columns shrinkable inside the outer day row", () => {
  ...
  import_node_assert.strict.match(dayLabel, /\.cb-week-day-summary/);
  import_node_assert.strict.match(dayLabel, /display:\s*flex/);
  import_node_assert.strict.match(dayLabel, /flex-wrap:\s*wrap/);
  import_node_assert.strict.match(dayLabel, /\.cb-week-day-metric/);
  import_node_assert.strict.match(dayLabel, /display:\s*inline-flex/);
  import_node_assert.strict.match(dayLabel, /white-space:\s*nowrap/);
  import_node_assert.strict.doesNotMatch(dayLabel, /overflow-wrap:\s*anywhere/);
